...
class GoGame:
    ...
    def train_model(self, num_epochs=50, lr=0.005, anneal_factor=0.95):
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch + 1) % 10 == 0: 
                print(f"E {epoch + 1}: Loss={total_loss:.3f}(Task:{task_loss:.3f},Reg:{reg_loss:.3f}) Tau={self.model.dsr_blocks[0].tau.item():.3f}")
    ...
    def _on_click(self, event):
        ...
        if 0 <= y < self.size and 0 <= x < self.size and self.play_move(y, x): 
            self._update_visualization()
    ...
    def _on_key_press(self, event):
        ...
        if key == 'u': 
            ...
            self._update_visualization()
        elif key in '123456789' and self.pyg_diagnostics and int(key) - 1 < len(self.pyg_diagnostics):
            self.active_layer_idx = int(key) - 1
            self._update_visualization()
        elif key == 'q': 
            plt.close(self.fig)
    ...
    def start(self):
        self._setup_visualization()
        self._update_visualization()
        print("Go UI|Keys: [t]rain, [g]o, [u]ndo, [r]eset, [c]lusters, [1/2..]layer, [q]uit")
        plt.show(block=True)
