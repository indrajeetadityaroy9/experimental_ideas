import os, subprocess, sys, textwrap, importlib.util, pathlib, re, shutil
...
run([sys.executable, "-m", "pip", "install", f"--index-url={idx}","torch==2.4.0", "torchvision==0.19.0", "torchaudio==2.4.0"])
...
run([sys.executable, "-m", "pip", "install", "torch-geometric==2.6.1"])

try:
    run([sys.executable, "-m", "pip", "install", "dgl==2.4.0","-f", "https://data.dgl.ai/wheels/torch-2.4/cu121/repo.html"])
except subprocess.CalledProcessError:
    run([sys.executable, "-m", "pip", "install", "dgl==2.4.0"])

run([sys.executable, "-m", "pip", "install", "--no-deps","git+https://github.com/BUPT-GAMMA/OpenHGNN@main"])
...
run([sys.executable, "-m", "pip", "install", "networkx>=3.2"])
